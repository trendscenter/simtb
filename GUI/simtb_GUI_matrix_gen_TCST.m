function varargout = simtb_GUI_matrix_gen_TCST(varargin)
% SIMTB_GUI_MATRIX_GEN_TCST M-file for simtb_GUI_matrix_gen_TCST.fig
%      SIMTB_GUI_MATRIX_GEN_TCST, by itself, creates a new SIMTB_GUI_MATRIX_GEN_TCST or raises the existing
%      singleton*.
%
%      H = SIMTB_GUI_MATRIX_GEN_TCST returns the handle to a new SIMTB_GUI_MATRIX_GEN_TCST or the handle to
%      the existing singleton*.
%
%      SIMTB_GUI_MATRIX_GEN_TCST('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in SIMTB_GUI_MATRIX_GEN_TCST.M with the given input arguments.
%
%      SIMTB_GUI_MATRIX_GEN_TCST('Property','Value',...) creates a new SIMTB_GUI_MATRIX_GEN_TCST or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before simtb_GUI_matrix_gen_TCST_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to simtb_GUI_matrix_gen_TCST_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help simtb_GUI_matrix_gen_TCST

% Last Modified by GUIDE v2.5 20-Jan-2011 21:59:44

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @simtb_GUI_matrix_gen_TCST_OpeningFcn, ...
    'gui_OutputFcn',  @simtb_GUI_matrix_gen_TCST_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before simtb_GUI_matrix_gen_TCST is made visible.
function simtb_GUI_matrix_gen_TCST_OpeningFcn(hObject, eventdata, handles, varargin)
set(handles.figure_gen, 'Color', [0 0 0])
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to simtb_GUI_matrix_gen_TCST (see VARARGIN)
...
if nargin < 2
    ...
    name = varargin{8};
    TCMtype = varargin{9}; 
    paneltitle = varargin{10};
    paramname = varargin{11}; 
    ...
    handles.TCMtype = TCMtype;
    
    mlist{1} = get(handles.modellist, 'String');
    for mm = 1:TCMtype
        mlist{mm+1} = num2str(mm);
    end
    set(handles.modellist, 'String', mlist)
end
...
function varargout = simtb_GUI_matrix_gen_TCST_OutputFcn(hObject, eventdata, handles)
...
% Determine the selected data set.
    ...
    case 'Random' %Matrix elements are Normal Distributed.
           set(handles.tag,'Visible','Off');
           set(handles.value,'Visible','Off'); 
           temp = ceil(handles.TCMtype*rand(handles.row, handles.col));
           set(handles.uitable1, 'data', temp);
           set(handles.uitable1,'Visible','On');  
           
    case 'Select Manually' % Matrix elements are Randomly Distributed.
           set(handles.tag,'Visible','Off');
           set(handles.value,'Visible','Off');  
...
%        str2double(get(hObject,'String')) returns contents of value as a double
 handles.uni_type = str2num(get(hObject,'String'));
 if  ~any(1:handles.TCMtype==handles.uni_type)
     Title = 'Error';
     Icon = 'error';
     Message = sprintf('Please choose integer value between 1 to %d', handles.TCMtype);
     h = msgbox(Message,Title,Icon);   
 else      
     temp = handles.uni_type * ones(handles.row, handles.col);
     set(handles.uitable1, 'data', temp);
     guidata(hObject,handles);
 end   
...
set(handles.model_desc, 'String', MDESC{str2num(str{val})})
% Save the handles structure.
guidata(hObject,handles)
